...
- a 'small' neural network is required for sensors in such a manner - as they run on smaller microcontrollers/computers with very much limited memory and must also perform neural network computations in the microseconds, as they are used to account for live sensor data. This requires using models limited or specialized for this taks (ex. GRU). This is also why we cannot entirely just use neural networks to compute the all of the sensor data - as the models would be too large and slow. The 'hybrid' solution of neural networks applied with Kalman filtering - is the current optimal, realistic approach.
- KalmanNET allows sensor data and signal conditioning to take the 'live environment' into account. Such as is 'nature' vs. 'nurture' in the adaptation of biological systems - where a combination of your genetic code and environment shapes your decision-making, KalmanNET allows us to introduce a mathematical hybrid of encoded patterns ('nature') with learned environmental factors ('nurture').
- concise all ref's of 'kalman-filter'